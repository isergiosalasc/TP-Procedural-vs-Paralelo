...
void gate_por_canal(char f_name[], float canal_v[GATES][PULSOS], float canal_h[GATES][PULSOS], int hilos);
void autocorrelacion(float canal_v[GATES][PULSOS], float canal_h[GATES][PULSOS], double p_autoc_v[], double p_autoc_h[], int hilos);
...
int main(int argc, char *argv[]) {
	...
	/* Configuraciones de openMP*/
	int hilos=1;
	if(argc>1)
	{
		hilos=atoi(argv[1]);
		if(hilos<1)
			hilos = omp_get_num_threads(); /* Se establece el numero max de hilos logicos en la PC*/
	}
	else
		hilos = omp_get_num_threads(); /* Se establece el numero max de hilos logicos en la PC*/
	...
	gate_por_canal(f_pulsos, canal_v, canal_h, hilos);

	autocorrelacion(canal_v, canal_h, autoc_v, autoc_h, hilos);
...
void gate_por_canal(char *f_name, float canal_v[GATES][PULSOS], float canal_h[GATES][PULSOS], int hilos){
	...
	/*Configuracion de hilos utilizados por openMP*/
  ...
  while(fread(&samples,1,sizeof(uint16_t), f_in)){
    ...
    {
		#pragma omp for schedule(static, porcion)
    ...
    {
    	case 1:	
		    	#pragma omp parallel firstprivate(i, gate, acumulador_v, acumulador_h, muestras_gate, error_total)
			    {
					#pragma omp sections
					{
						#pragma omp section /* Un hilo se encarga de los calculos del canal v*/
						{
					    	for (i = 0; i < samples; i++) {
						...
						#pragma omp section /* Otro hilo se encarga de los calculos del canal h*/
						{
					    	for (i = 0; i < samples; i++) {
    			...
    			break;
    	case 2:
				#pragma omp parallel firstprivate(i, gate, acumulador_v, acumulador_h, muestras_gate)
			    {
					#pragma omp sections
					{
						#pragma omp section /* Un hilo se encarga de los calculos del canal v*/
...
void autocorrelacion(float canal_v[GATES][PULSOS], float canal_h[GATES][PULSOS], double p_autoc_v[], double p_autoc_h[], int hilos){
	...
	/* Configuraciones de paralelizacion con openMP*/
	omp_set_num_threads(hilos);
	int porcion= GATES/hilos;

	autoc_i=omp_get_wtime();
	
	#pragma omp parallel firstprivate(i, j, temp_v, temp_h)
    {
		#pragma omp for schedule(static, porcion)
		for (i = 0; i < GATES; i++) {
